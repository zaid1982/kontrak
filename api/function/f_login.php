...
class Class_login {
    ...
    public function create_jwt ($userId='', $username='') {
        try {
            ...
            $key = "gems2";
            $token = array('iss'=>'spdp/jwt', 'userId'=>$userId, 'username'=>$username, 'iat'=>time(), 'exp'=>time()+10);
    ...
    public function get_menu_list ($arr_roles=array()) {
        try {
            ...
            foreach ($menu_list as $menu) {
                //$this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'nav_page = '.$menu['navPage']);
                //$this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'nav_index = '.$nav_index);
                if (empty($menu['navSecondId'])) {
                    array_push($menu_return, array('navId'=>$menu['navId'], 'navDesc'=>$menu['navDesc'], 'navIcon'=>$menu['navIcon'], 'navPage'=> $this->fn_general->clear_null($menu['navPage']), 'navSecond'=>array()));
                    $nav_index++;
                } else {
                    array_push($menu_return[$nav_index-1]['navSecond'], array('navSecondId'=>$menu['navSecondId'], 'navSecondDesc'=>$menu['navSecondDesc'], 'navSecondPage'=>$menu['navSecondPage']));
                }
            }
            return $menu_return;
        }
    ...
    public function check_login ($username, $password, $deviceId='') {
        try {
            ...
            if ($profile['userPassword'] !== md5($password)) {
                $timeBlock = $profile['userFailAttempt'] >= '2' ? 'Now()' : '';
            ...
            if ($profile['userFailAttempt'] >= '3') {
                if (!empty($profile['minuteBlock']) && $profile['minuteBlock'] <= '10') {
            ...
            $result['token'] = $token;
            $result['isFirstTime'] = is_null($profile['userTimeActivate']) ? 'Yes' : 'No';
            $result['roles'] = $arr_roles;
            if (!empty($profile['uploadId'])) {
                $upload = Class_db::getInstance()->db_select_single('vw_sys_upload', array('upload_id'=>$profile['uploadId']), null, 1);
                $result['imgUrl'] = $constant::URL.$upload['uploadFolder'].'/'.$upload['uploadFilename'].'.'.$upload['uploadExtension'];
            } else {
                $result['imgUrl'] = '';
            }
     ...
     * @throws Exception
     */
    public function check_login_web ($params=array()) {
        try {
            $this->fn_general->log_debug(__CLASS__, __FUNCTION__, __LINE__, 'Entering '.__FUNCTION__);
            $constant = $this->constant;

            $this->fn_general->checkEmptyParamsArray($params, array('username', 'password'));

            $profile = Class_db::getInstance()->db_select_single('vw_profile', array('user_name'=>$params['username']));
            if (empty($profile)) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_LOGIN_NOT_EXIST, 31);
            }
            if ($profile['userPassword'] !== md5($params['password'])) {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_LOGIN_WRONG_PASSWORD, 31);
            }
            if ($profile['userStatus'] === '2') {
                throw new Exception('[' . __LINE__ . '] - '.$constant::ERR_LOGIN_NOT_ACTIVE, 31);
            }

            $userId = $profile['userId'];
            $result = $this->fn_general->convertDbIndex($profile);

            $arr_roles = Class_db::getInstance()->db_select('vw_roles', array(), null, null, null, array('user_id'=>$userId));
            $token = $this->create_jwt($userId, $params['username']);
            $result['token'] = $token;
            $result['roles'] = $arr_roles;
